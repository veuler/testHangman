...
__TITLE__="HANGMAN"
__AUTHOR__="##Veysel Nantu##"
__DATE__="28.11.2014"


simge="-"*50
bos=""*5
bilgi="""            You will earn 50 points if you guess correctly
            and you will lose 25 points if your guess is incorrect.
            If you fail 5 times the game is over."""
print("{:^80}\n{:^80}\n{:^80}\n{:^80}\n{}".format(__TITLE__,__AUTHOR__,__DATE__,simge,bos))
print(bilgi)
...
for i in tablov:
    for h in i:
        tablo.append(h)
print("The word has {} letters.".format(len(secim)),' '.join(tablo))
print(""*7)
c="""  _________
  |
  |"""
...
while True:
    ...
    if can==0:
        ...
        sleep(0.9)
        print ("The word that you couldnt guess: {}".format(secim))
        sleep(0.9)
        print("Please press 'Enter' for quit..")
        break
    ...
    if x in secim and not x in depo:
        ...
        for sayi, oge in enumerate(secim):
            if 2 <=secim.count(oge) and x==oge:
                ...
                if tablo[sayi]=="-":
                    tablo[sayi]=x
                print("At position {} .".format(sayi+1))
                sleep(0.7)
        ...
        can-=1
        puan-=25
        print("\nThis letter {} not in the word! {} guesses left.".format(x,can))
        sleep(0.7)
        print("\nWord:",' '.join(tablo))
        sleep(0.7)
        if can==4:
